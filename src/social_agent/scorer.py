"""社媒内容打分引擎 — 7维评分 + 盲预测 + 复盘。改编自 Cheat on Content。"""
...
RUBRIC_SYSTEM = """你是社媒内容专业评委。你的任务是对一篇社媒内容进行 7 维评分。
...
0: 纯信息表面 | 3: 触到现象但无新视角 | 5: 命名了读者认识但无语言形容的模式
...
0: 极窄(需专业背景) | 3: 中等(泛从业者) | 5: 普世(圈外人也看得懂)

### TS — Topic Shareability（分享冲动，权重 ×1.0）
转发是否暴露转发者的处境或品味？
1: 暴露焦虑/恐惧 | 3: 安全中性 | 5: 转发即表演(身份信号/品味宣言)
